...
func (bp *blockProvider) RunBackup(
	...
	if patterns, err := uploaderutil.GetExclude(uploaderCfg); err != nil {
		return "", false, 0, 0, 0, errors.Wrap(err, "failed to read Exclude uploader config")
	} else if len(patterns) > 0 {
		return "", false, 0, 0, 0, fmt.Errorf("exclude is not supported by data mover %q", uploader.BlockType)
	}
