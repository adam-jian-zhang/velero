...
func DescribeBackup(
	...
	return Describe(func(d *Describer) {
		...
		if backup.Spec.ResourcePolicy != nil {
			...
			// Display namespace-scoped filter policies if they exist
			DescribeNamespaceScopedFilterPolicies(ctx, kbClient, d, backup)
		}
...
// DescribeNamespaceScopedFilterPolicies describes namespace-scoped filter policies if present
func DescribeNamespaceScopedFilterPolicies(ctx context.Context, kbClient kbclient.Client, d *Describer, backup *velerov1api.Backup) {
	if backup.Spec.ResourcePolicy == nil {
		return
	}
