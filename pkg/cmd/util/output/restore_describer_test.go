...
func TestDescribeRestore_OwnerReferenceRemapping_QualifiedRemediation(t *testing.T) {
	kbClient := velerotest.NewFakeControllerRuntimeClient(t)
	restore := builder.ForRestore("velero", "test-restore").
		Backup("test-backup").
		Result()
	...
	out := DescribeRestore(context.Background(), kbClient, restore, nil, false, false, "")
	assert.Contains(t, out, "kubectl annotate Cluster.cluster.x-k8s.io -n prod-ns prod-cluster cluster.x-k8s.io/paused-")
	assert.Contains(t, out, "kubectl label Cluster.cluster.x-k8s.io -n prod-ns prod-cluster velero.io/quiesced-by-restore-")
	assert.Contains(t, out, "kubectl annotate ConfigMap cluster-cm example.io/paused-")
	assert.Contains(t, out, "kubectl label ConfigMap cluster-cm velero.io/quiesced-by-restore-")
}

func TestDescribeRestore_OwnerReferenceRemapping_OverflowConfigMap(t *testing.T) {
	kbClient := velerotest.NewFakeControllerRuntimeClient(t)
	restore := builder.ForRestore("velero", "test-restore").
		Backup("test-backup").
		Result()
