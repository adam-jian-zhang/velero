...
- namespaces: ["test"]
  ...
  - kinds: []
    labelSelector:
      app: db`,
			wantErr: true,
			errMsg: "only one resource filter with empty kinds is allowed",
		},
		{
...
- namespaces: ["test"]
  ...
  - kinds: []
    ...
    labelSelector:
      app: web`,
			wantErr: true,
			errMsg: "names or excludedNames cannot be specified when kinds is empty",
		},
		{
  ...
  - kinds: []
    ...
    labelSelector:
      app: web`,
		...
		{
			name: "invalid - no label selectors with empty kinds",
			yamlData: `version: v1
namespacedFilterPolicies:
- namespaces: ["test"]
  resourceFilters:
  - kinds: []`,
			wantErr: true,
			errMsg: "labelSelector or orLabelSelectors must be specified when kinds is empty",
		},
		{
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.wantErr {
				...
				assert.True(t, len(nfPolicies) >= 1) // Valid test cases have at least 1 policy
			}
		})
	}
}
