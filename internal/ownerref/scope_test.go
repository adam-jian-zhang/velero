...
func TestScopeDenyList(t *testing.T) {
	...
	// Top-level workloads are NOT denied, but are also not in-scope by default (seeds only have CAPI/KubeVirt)
...
func TestScopeBuiltInSeeds(t *testing.T) {
	...
	// KubeVirt is no longer in built-in seeds (separated to follow-up design)
	kubevirtGVK := schema.GroupVersionKind{Group: "kubevirt.io", Version: "v1", Kind: "VirtualMachine"}
	assert.False(t, scope.IsInScope(kubevirtGVK), "KubeVirt should not be in built-in seeds")
