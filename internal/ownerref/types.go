...
// ScopeEntry identifies a Group and optional Kind that is in-scope for ownerRef relinking.
type ScopeEntry struct {
	Group string `yaml:"group" json:"group"`
	Kind  string `yaml:"kind,omitempty" json:"kind,omitempty"`
}

// SpecRefPathEntry configures object-reference-like spec field paths to relink for a GVK.
...
// OwnerRefRelinkState tracks dynamic UID mappings and queues during restore.
type OwnerRefRelinkState struct {
...
// OwnerRefRemapState is an alias for OwnerRefRelinkState for backward compatibility.
type OwnerRefRemapState = OwnerRefRelinkState

// NewOwnerRefRelinkState creates a new initialized OwnerRefRelinkState.
func NewOwnerRefRelinkState() *OwnerRefRelinkState {
	return &OwnerRefRelinkState{
...
// NewOwnerRefRemapState creates a new initialized OwnerRefRelinkState (backward-compatible alias).
func NewOwnerRefRemapState() *OwnerRefRelinkState {
	return NewOwnerRefRelinkState()
}

// RegisterUIDMapping records an oldUID -> newUID mapping in a thread-safe manner.
func (s *OwnerRefRelinkState) RegisterUIDMapping(oldUID, newUID types.UID) {
...
// GetNewUID retrieves the relinked newUID for a given oldUID.
func (s *OwnerRefRelinkState) GetNewUID(oldUID types.UID) (types.UID, bool) {
	if s == nil || oldUID == "" {
		return "", false
	}
	s.uidMapLock.RLock()
...
// IsNewUID returns true if the given UID is known to be a relinked newUID.
func (s *OwnerRefRelinkState) IsNewUID(uid types.UID) bool {
	if s == nil || uid == "" {
		return false
	}
	s.uidMapLock.RLock()
...
func (s *OwnerRefRelinkState) EnqueueOwnerPatch(req OwnerPatchRequest) {
	if s == nil {
		return
	}
	s.queueLock.Lock()
...
func (s *OwnerRefRelinkState) EnqueueSpecPatch(req OwnerPatchRequest) {
	if s == nil {
		return
	}
	s.queueLock.Lock()
...
func (s *OwnerRefRelinkState) RecordQuiescedObject(rec QuiescedObjectRecord) {
	if s == nil {
		return
	}
	s.queueLock.Lock()
...
func (s *OwnerRefRelinkState) RecordQuiescedObjectWithUID(oldUID types.UID, rec QuiescedObjectRecord) {
	if s == nil {
		return
	}
...
func (s *OwnerRefRelinkState) RegisterParentUID(childOldUID, parentOldUID types.UID) {
...
func (s *OwnerRefRelinkState) ResolveQuiescedRoots(req OwnerPatchRequest) []velerov1api.TargetRef {
	if s == nil {
		return nil
	}
	s.uidMapLock.RLock()
...
func (s *OwnerRefRelinkState) GetOwnerPatchQueue() []OwnerPatchRequest {
	if s == nil {
		return nil
	}
	s.queueLock.Lock()
...
func (s *OwnerRefRelinkState) SetOwnerPatchQueue(q []OwnerPatchRequest) {
	if s == nil {
		return
	}
	s.queueLock.Lock()
...
func (s *OwnerRefRelinkState) GetSpecPatchQueue() []OwnerPatchRequest {
	if s == nil {
		return nil
	}
	s.queueLock.Lock()
...
func (s *OwnerRefRelinkState) SetSpecPatchQueue(q []OwnerPatchRequest) {
	if s == nil {
		return
	}
	s.queueLock.Lock()
...
func (s *OwnerRefRelinkState) GetQuiescedObjects() []QuiescedObjectRecord {
	if s == nil {
		return nil
	}
	s.queueLock.Lock()
...
func (s *OwnerRefRelinkState) BlockUnquiesceForTarget(group, kind, namespace, name string) {
	if s == nil {
		return
	}
	s.queueLock.Lock()
...
func (s *OwnerRefRelinkState) GetSpecRefPaths() []SpecRefPathEntry {
	if s == nil {
		return nil
	}
	s.queueLock.Lock()
...
func (s *OwnerRefRelinkState) SetScope(scope *Scope) {
	if s == nil {
		return
	}
	s.queueLock.Lock()
...
func (s *OwnerRefRelinkState) GetScope() *Scope {
	if s == nil {
		return nil
	}
	s.queueLock.Lock()
