...
2. **Per-namespace filter lookup.** For each namespace that passes the global namespace filter, the system checks whether any `namespacedFilterPolicies` entry matches (by exact name, glob pattern, or namespace label selector). If a match is found, the `resourceFilters` array determines what gets backed up for that namespace:
...
namespacedFilterPolicies:
  - namespaces: ["test-namespace"]
    resourceFilters: []  # empty array or omitted entirely
```
**Behavior:** Valid configuration. It means **no resource filters** are applied to this namespace, effectively overriding any restrictive global filters to **backup all resources** in this namespace. If you want to entirely exclude a namespace, use the `velero.io/exclude-from-backup: "true"` label on the namespace instead.
...
1. **Each filter policy must specify at least one inclusion criteria:**
   ```
   namespacedFilterPolicies[N]: at least one of 'namespaces' or 'namespaceLabelSelector' must be specified
   ```

2. **Namespace label selectors must be valid:**
   ```
   namespacedFilterPolicies[N]: invalid namespaceLabelSelector: "invalid label key!" is not a valid label key
   namespacedFilterPolicies[N]: invalid excludedNamespaceLabelSelector: "invalid label key!" is not a valid label key
   ```

3. **Action validation:**
   ```
   namespacedFilterPolicies[N]: action must be either 'Include' or 'Skip'
   namespacedFilterPolicies[N]: resourceFilters must be empty when action is 'Skip'
   ```

4. **Each resource filter without kinds can only be defined once, and cannot specify names/excludedNames. Additionally, it must specify either labelSelector or orLabelSelectors:**
   ```
...
5. **No duplicate kinds across resource filter entries** within the same namespace filter:
   ```
   namespacedFilterPolicies[N]: kind "Pod" appears in both resourceFilters[0] and resourceFilters[2]
   ```

6. **`labelSelector` and `orLabelSelectors` mutual exclusion** within each resource filter:
   ```
   namespacedFilterPolicies[N].resourceFilters[M]: labelSelector and orLabelSelectors cannot co-exist
   ```

7. **No duplicate namespace patterns across filter policies.** This validates only exact duplicates - runtime behavior handles overlapping patterns.
...
8. **Namespace patterns must be valid globs.**

9. **Resource name patterns must be valid globs.**

10. **Resource kind validation with discovery helper** (performed during backup initialization).
...
### Use labels to assign namespaces to different backup schedules

**Goal:** You want to run backups on different schedules (e.g., daily vs. weekly) based on namespace labels. Some namespaces are labeled `velero-backup-schedule: daily`, while others have `velero-backup-schedule: weekly`.

You can achieve this by creating two separate `ResourcePolicy` ConfigMaps and two separate Velero `Schedule` objects. By using a "catch-all" skip rule at the bottom of each policy, you ensure only the targeted namespaces are backed up.

**ResourcePolicy ConfigMap 1 (Daily):**
```yaml
version: v1
namespacedFilterPolicies:
  # 1. Target the daily namespaces and back up everything
  - namespaceLabelSelector:
      velero-backup-schedule: daily
    # resourceFilters omitted: back up all resources in these namespaces

  # 2. Catch-all: Skip every other namespace
  - namespaces: ["*"]
    action: Skip
```
...
spec:
  ...
  template:
    includedNamespaces: ["*"] # Global inclusion, but policy handles the actual filtering
    resourcePolicy:
      kind: configmap
      name: daily-filter-policy
```

**ResourcePolicy ConfigMap 2 (Weekly):**
```yaml
version: v1
namespacedFilterPolicies:
  # 1. Target the weekly namespaces and back up everything
  - namespaceLabelSelector:
      velero-backup-schedule: weekly
    # resourceFilters omitted: back up all resources
...
spec:
  ...
  template:
    ...
    resourcePolicy:
      kind: configmap
      name: weekly-filter-policy
```

### Exclude namespaces with a certain label from the backup ENTIRELY

**Goal:** You want to back up all namespaces, but completely **skip/exclude** any namespace with a specific label from the backup archive.

**ResourcePolicy ConfigMap:**
```yaml
version: v1
namespacedFilterPolicies:
  # Target the namespaces we want to completely restrict
  - namespaceLabelSelector:
      exclude-entirely: "true"
    action: Skip

  # For everything else, apply normal filters (or back up everything)
  - namespaces: ["*"]
    resourceFilters:
      - kinds: [ConfigMap, Secret, Deployment]
```

### Filter using multiple label conditions (OR logic) with exclusions

**Goal:** You want to back up **all resources** for namespaces that belong to either `team: platform` OR `team: infra` (overriding restrictive global filters), but you also want to make sure any namespace labeled `confidential: "true"` is excluded from this policy, regardless of its team.

Because `namespaceLabelSelector` uses AND logic for the labels within its map, expressing an OR condition requires defining multiple policy entries.

**ResourcePolicy ConfigMap:**
```yaml
version: v1
namespacedFilterPolicies:
  # First condition: team = platform
  - namespaceLabelSelector:
      team: platform
    excludedNamespaceLabelSelector:
      confidential: "true"
